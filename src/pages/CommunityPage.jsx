...
const CHANNELS = [
  ...
  { name: '#language-exchange', desc: 'Somali · Amharic · Swahili.', members: 131 },
...
const FEATURED_THREADS = [
  {
    ...
    title: 'The Great Xalwo Debate: ghee or oil?',
    replies: 52,
    by: 'Aminah',
    snippet:
      '"My mum swears by ghee. My aunt insists on oil. Let\'s settle this once."',
  },
  {
    ...
    by: 'Daniel',
    snippet:
      '"Landing in Mogadishu, the sea hit me before I even left the tarmac..."',
  },
  {
    channel: '#listening-room',
    title: 'April playlist: Somali oldies & Ethiopian jazz',
    replies: 27,
    by: 'Ibrahim',
...
export default function CommunityPage() {
  ...
  return (
    <>
          ...
          <Reveal className="lg:col-span-4">
            <div className="bg-paper border-2 border-maroon/30 p-6 md:p-8 pixel-shadow">
              <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-maroon/60 mb-4 inline-flex items-center gap-2">
                <span className="w-2 h-2 bg-y2klime animate-blink" /> Live
              </p>
              <div className="flex items-center gap-3 mb-6">
                <p className="font-display text-3xl text-maroon">247 online</p>
              </div>
              <p className="text-sm text-charcoal/85 leading-relaxed">
                Daily chatter across twelve channels — xalwo debates, playlist
                swaps, language practice, and the running joke about who really
                gets to claim bariis.
              </p>
              <a
                href="#/community"
                className="mt-6 inline-flex w-full items-center justify-center gap-2 px-6 py-3.5 font-pixelbold text-[11px] uppercase tracking-widest2 bg-maroon text-bone hover:bg-rust transition-colors duration-300 pixel-shadow-cream"
              >
                Join the Discord <PixelArrow size={14} />
              </a>
            </div>
          </Reveal>
          ...
          <div className="lg:col-span-8">
            <Reveal>
              <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-maroon/60 mb-3 inline-flex items-center gap-2">
                <PixelStar size={10} className="text-maroon" /> What happens inside
              </p>
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {CHANNELS.map((c, i) => (
                <Reveal key={c.name} delay={(i % 4) * 60}>
                  <div className="bg-paper border-2 border-maroon/20 p-5">
                    ...
                    <div className="mt-4 space-y-1.5">
                      <div className="h-2 w-full bg-maroon/10" />
                      <div className="h-2 w-5/6 bg-maroon/10" />
                      <div className="h-2 w-4/6 bg-maroon/10" />
                    </div>
                  </div>
                </Reveal>
              ))}
      ...
      <section className="bg-paper py-20 md:py-28">
        <div className="px-6 md:px-10 max-w-[1400px] mx-auto">
          <div className="flex items-end justify-between mb-12">
            <Reveal>
              <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-maroon/60 mb-3 inline-flex items-center gap-2">
                <PixelStar size={10} className="text-maroon animate-sparkle" /> Featured conversations · updated weekly
              </p>
              ...
              <Reveal as="li" key={t.title} delay={i * 80}>
                <a href="#/community" className="grid grid-cols-12 gap-4 md:gap-8 py-7 md:py-9 group">
                  ...
                  <div className="col-span-12 md:col-span-2 md:text-right">
                    ...
                    <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-maroon mt-2 border-b-2 border-maroon inline-flex items-center gap-1">
                      Read <PixelArrow size={10} />
                    </p>
                  </div>
                </a>
              </Reveal>
      ...
      <section className="bg-rust text-cream py-24 md:py-32 relative">
        <div className="absolute inset-0 crt-scan opacity-30 pointer-events-none" />
        <div className="relative px-6 md:px-10 max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-14">
          <div className="lg:col-span-5">
            <Reveal>
              <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-gold mb-5 inline-flex items-center gap-2">
                <PixelHeart size={10} className="text-gold animate-sparkle" /> Share your story
              </p>
            </Reveal>
            <Reveal delay={80}>
          ...
          <Reveal delay={180} className="lg:col-span-7">
            {submitted ? (
              <div className="bg-cream text-rust p-8 md:p-10 border-2 border-gold pixel-shadow-cream">
                <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-maroon/60 mb-4 inline-flex items-center gap-2">
                  <PixelHeart size={10} className="text-maroon" /> Received with thanks
                </p>
              ...
              <form
                onSubmit={onSubmit}
                className="bg-rust border-2 border-cream/30 p-6 md:p-10 space-y-6"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <label className="flex flex-col gap-2">
                    <span className="font-pixelbold text-[9px] uppercase tracking-widest2 text-gold">Your name</span>
                    <input
                      required
                      type="text"
                      className="bg-transparent border-b border-cream/30 py-2 focus:outline-none focus:border-gold placeholder:text-cream/40 font-pixel text-[16px]"
                      placeholder="How you sign letters"
                    />
                  </label>
                  <label className="flex flex-col gap-2">
                    <span className="font-pixelbold text-[9px] uppercase tracking-widest2 text-gold">Heritage</span>
                    <input
                      required
                      type="text"
                      className="bg-transparent border-b border-cream/30 py-2 focus:outline-none focus:border-gold placeholder:text-cream/40 font-pixel text-[16px]"
                ...
                <label className="flex flex-col gap-2">
                  <span className="font-pixelbold text-[9px] uppercase tracking-widest2 text-gold">Story title</span>
                  <input
                    required
                    type="text"
                    className="bg-transparent border-b border-cream/30 py-2 focus:outline-none focus:border-gold placeholder:text-cream/40 font-pixel text-[16px]"
                ...
                <label className="flex flex-col gap-2">
                  <span className="font-pixelbold text-[9px] uppercase tracking-widest2 text-gold">The story (draft or full)</span>
                  <textarea
                    required
                    rows="6"
                ...
                <button
                  type="submit"
                  className="w-full md:w-auto inline-flex items-center gap-2 px-8 py-3.5 font-pixelbold text-[11px] uppercase tracking-widest2 bg-bone text-maroon hover:bg-gold hover:text-rust transition-colors duration-300 pixel-shadow-cream"
                >
                  Send story <PixelArrow size={14} />
                </button>
              </form>
            )}
          </Reveal>
      ...
      <section className="bg-bone py-20 md:py-28">
        <div className="px-6 md:px-10 max-w-[1400px] mx-auto">
          <Reveal>
            <p className="font-pixelbold text-[10px] uppercase tracking-widest2 text-maroon/60 mb-3 inline-flex items-center gap-2">
              <PixelStar size={10} className="text-maroon animate-sparkle" /> Customer gallery
            </p>
            <h2 className="font-display font-light text-3xl md:text-5xl text-maroon leading-tight max-w-[20ch]">
              Tag your orders. <span className="italic">#nomadhousehome</span>
            </h2>
          </Reveal>
